...
Deno.test("UserService.updateUsername", async (t) => {
  ...
  await t.step("should update username if not taken", async () => {
    ...
    findOneAndUpdateStub = sinon.stub(User, "findOneAndUpdate").resolves({
      ...testUser,
...
Deno.test("UserService.uploadAvatar", async (t) => {
  ...
  await t.step("should upload avatar successfully", async () => {
    ...
    findByIdStub = sinon.stub(User, "findById").resolves(testUser);
    saveImageFileStub = sinon.stub(ImageService, "saveImageFile").resolves(
      "path/to/new-avatar.jpg",
    );
    deleteImageStub = sinon.stub(ImageService, "deleteImage").resolves(
      undefined,
    );
    ...
    assertEquals(
      ...
      "http://example.com/path/to/new-avatar.jpg",
    );
  ...
  await t.step("should throw 500 if unable to save avatar", async () => {
    ...
    findByIdStub = sinon.stub(User, "findById").resolves(testUser);
    saveImageFileStub = sinon.stub(ImageService, "saveImageFile").rejects(
  ...
  await t.step("should attempt to delete old avatar if present", async () => {
    const testUser = {
      id: "1",
      username: "testuser",
      avatarUrl: "http://example.com/oldavatar.jpg",
      save: sinon.stub().resolvesThis(),
    };
    ...
    assertEquals(
      updatedUser.avatarUrl,
      "http://example.com/path/to/avatar.jpg",
    );
  ...
  await t.step(
    "should log and not throw error if deleting old avatar fails",
    async () => {
      const testUser = {
        id: "1",
        username: "testuser",
      ...
      deleteImageStub = sinon.stub(ImageService, "deleteImage").rejects(
...
Deno.test("UserService.deleteAvatar", async (t) => {
  ...
  await t.step(
    "should not delete avatar if there is no avatar URL",
    async () => {
      const testUser = {
        id: "1",
        username: "testuser",
  ...
  await t.step(
    "should log and not throw error if deleting avatar fails",
    async () => {
      const testUser = {
        id: "1",
        username: "testuser",
