...
import { assertEquals, assertNotEquals, assertRejects, assertStrictEquals, assertThrows } from "https://deno.land/std@0.106.0/testing/asserts.ts";
...
Deno.test("AuthService.register", async (t) => {
    ...
    await t.step("returns jwt if successfull", async () => {
        ...
        sendVerificationEmail = sinon.stub(EmailUtils, "sendVerificationEmail").resolves();

        try {
            const returnedJwt = await AuthService.register("test.test@gmail.com", "test", "password");
    ...
    await t.step("throws 400 error, username already exsits", async () => {
        ...
        try {
            await assertRejects(
                async () => await AuthService.register("test.test@gmail.com", "test", "password"),
                HttpError,
                "already exists",
            );
        } finally {
    ...
    await t.step("not allowed Username, throws error", async () => {
        ...
        for (const username of invalidUsernames) {
            try {
                await assertRejects(
                    async () => await AuthService.register("test.test@gmail.com", username, "password"),
                    HttpError,
                );
            } finally {
                findOneStub.restore();
...
Deno.test("AuthService.login", async (t) => {
    ...
    await t.step("returns jwt if successful", async () => {
        ...
        try {
            const returnedJwt = await AuthService.login("test.test@gmail.com", "password");
            assertEquals(returnedJwt, jwt);
            assert(createJWT.calledOnce)
        } finally {
            findOneStub.restore();
    ...
    await t.step("wrong password, return invidial credentials", async () => {
        ...
        try {
            await assertRejects(
                async () => await AuthService.login("test.test@gmail.com", "wrongPassword"),
                HttpError,
                "Invalid credentials"
            );
        } finally {
...
Deno.test("AuthService.requestPasswordReset", async (t) => {
    ...
    await t.step("successfully set password reset token and send mail", async () => {
    ...
    await t.step("no user found, throw 404 user not found error", async () => {
        ...
        try {
            await assertRejects(
                async () => await AuthService.requestPasswordReset("wrongEmail@gmail.com"),
                HttpError,
...
Deno.test("AuthService.resetPassword", async (t) => {
    ...
    await t.step("successfully resets password when token is valid", async () => {
        ...
        const hashedTokenBuffer = await crypto.subtle.digest("SHA-256", encodedToken);
...
Deno.test("UserService.resendVerificationCode", async (t) => {
    ...
    await t.step("should send a new verification code and save user", async () => {
        const testUser = {
            id: "1",
        ...
        const sendEmailStub = sinon.stub(EmailUtils, "sendVerificationEmail").resolves();
